(() => {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const updateModalOpenButtons = document.querySelectorAll('.update-modal-link');
        ...
        const openModalHandler = (btn) => {
            ...
            const rowCells = document.querySelectorAll(`td[data-id="${tableName}_${rowId}"]`);
            rowCells.forEach((cell) => {
                ...
                // Добавляем обработчик события input для обновления href
                inputField.addEventListener('input', () => updateSubmitButtonHref(tableName, rowId));
                
                label.append(labelText, inputField);
                modalInputs.appendChild(label);
            });

            // Изначально устанавливаем href у кнопки отправки
            updateSubmitButtonHref(tableName, rowId);
        }
        
        Array.from(updateModalOpenButtons).forEach((btn) => btn.addEventListener('click', (e) => {
            e.preventDefault();
            openModalHandler(btn);
        }));

        updateModal.addEventListener('click', (e) => e.target.id === 'update_modal' && closeModalHandler());
